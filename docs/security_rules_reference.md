...
This document contains the commented code and development notes extracted from `static_rules.py` during the project cleanup.

## Original Rule Definitions (Commented)

### Basic Rule Implementation - Version 1
```python
...
### Enhanced Rule Definitions - Version 2
```python
# Assuming flow has a .request attribute with .host, .headers
# and the STATIC_RULES list is available in the scope.
...
1. **ALERT** - Critical threats requiring immediate attention
2. **BLOCK** - High-risk activities that should be prevented
3. **REJECT** - Policy violations that should be denied
4. **WARN** - Suspicious activities requiring monitoring
5. **LOG** - Informational events for audit trail
6. **ALLOW** - Normal traffic

### Development Notes

- Original threshold for large uploads was 200KB, later increased to 500KB
- C2 indicators include whitelist exceptions for Microsoft and Google APIs
- Internal IP patterns cover common private IP ranges and localhost variants
- XSS payload detection covers common injection vectors
- User-Agent filtering targets known scanning tools and automated clients
