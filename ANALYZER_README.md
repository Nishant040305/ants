# MITM Proxy Log Analyzer with Google Gemini

This tool analyzes mitmproxy logs using Google Gemini to detect security issues in HTTP traffic.

## Prerequisites

1. Python 3.7+
2. Google API key with access to Gemini
3. mitmproxy logs in JSONL format (generated by `save_flow.py`)
